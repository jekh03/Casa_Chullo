...
const AnimatedBox = () => {
  ...
  const [headStyles, headApi] = useSpring(() => ({
    from: { x: 120, width: '200px', height: '200px', backgroundColor: '#61dafb',filter:'blur(1px)', zIndex:2},
  }));

  const [bodyStyles, bodyApi] = useSpring(()=>({
    from: {x:0, width: '100px', height: '100px', backgroundColor: '#61dafb', position: 'absolute',filter:'blur(2px)',zIndex:1},
  }));

  const [bodyStyles2, bodyApi2] = useSpring(()=>({
    from: {x:340, width: '300px', height: '300px', backgroundColor: '#61dafb', position: 'absolute',filter:'blur(0px)',zIndex:3},
  }));

  const [bodyStyles3, bodyApi3] = useSpring(()=>({
    from: {x:660, width: '200px', height: '200px', backgroundColor: '#61dafb', position: 'absolute',filter:'blur(1px)',zIndex:2},
  }));

  const [bodyStyles4, bodyApi4] = useSpring(()=>({
    from: {x:880, width: '100px', height: '100px', backgroundColor: '#61dafb', position: 'absolute',filter:'blur(2px)',zIndex:1},
  }));
  ...
  const handleClick = () => {
    if (step === 0) {
      ...
      headApi.start({
        to: { x: 340, width: '300px', height: '300px', backgroundColor: '#61dafb',filter:'blur(0px)',zIndex:3},
      });

      bodyApi.start({
        to: {x:120, width: '200px', height: '200px', backgroundColor: '#61dafb',filter:'blur(1px)',zIndex:2}
      });

      bodyApi2.start({
        to: {x:660, width: '200px', height: '200px', backgroundColor: '#61dafb',filter:'blur(1px)',zIndex:2}
      });

      bodyApi3.start({
        to: {x:880, width: '100px', height: '100px', backgroundColor: '#61dafb',filter:'blur(2px)',zIndex:1}
      });

      bodyApi4.start({
        to: {x:0, width: '100px', height: '100px', backgroundColor: '#61dafb',filter:'blur(2px)',zIndex:1}
      });
      setStep(1);
    } else if (step === 1) {
